        ...
        <div class="row mb-4">
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <h1><i class="fas fa-edit me-2"></i>Editar Usuario</h1>
                        <p class="text-muted">Modificar información del usuario: <strong>{{ $user->name }}</strong></p>
                    </div>
                    <div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('users.update', $user->id) }}" method="POST">
                            ...
                            <div class="mb-3" id="colegio_assignment_div" style="{{ old('role', $user->role) == 'admin' || old('role', $user->role) == 'supervisor' ? 'display:block;' : 'display:none;' }}">
                                ...
                                <select class="form-control @error('colegio_id') is-invalid @enderror" 
                                    ...
                                    @foreach($colegios as $colegio)
                                        <option value="{{ $colegio->id }}" {{ old('colegio_id', $user->colegio_id) == $colegio->id ? 'selected' : '' }}>
                                            {{ $colegio->nombre }} 
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <small class="text-muted">
                                            <strong>Colegio actual:</strong> {{ $user->colegio ? $user->colegio->nombre : 'Sin asignar' }}
                                        </small>
                                    </div>
                                    <div class="col-md-6">
                                        <small class="text-muted">
                                            <strong>Municipio:</strong> {{ $user->colegio && $user->colegio->municipio ? $user->colegio->municipio->nombre : 'N/A' }}
                                        </small>
                                    </div>
                                </div>
                                <div class="row mt-2">
                            ...
                            @if($user->id === Auth::id())
                                <div class="alert alert-info">
                                    ...
                                    <strong>Nota:</strong> Estás editando tu propio perfil. Ten cuidado al cambiar el rol o el colegio asignado.
                                </div>
                            @endif
